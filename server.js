...
function requestHttpAccess(res) {
  res.writeHead(401, responseHeaders({
    ...
    'www-authenticate': 'Basic realm="Host Control", charset="UTF-8"'
  }));
...
async function typeMissionTextAndConfirm(target, session, pane, textValue, marker, submitKey = 'C-m') {
  return typeMarkedTextAndConfirm(target, session, pane, textValue, marker, {
    ...
    renderedPredicate: (output) => {
      ...
      return renderedText.includes(`[Host Control Mission `) && renderedText.includes(marker);
    }
  });
}
...
function workspaceAreaName(currentPath) {
  ...
  if (resolved === __dirname || resolved.startsWith(`${__dirname}${path.sep}`)) return 'Host Control';
...
function reviewSummary(review) {
  const lines = usefulOutputLines(review?.lastOutput || '')
    ...
    .filter((line) => !/dedicated (?:Host Control|Agent Orchestrator) review agent|generated context file|latest-context\.md/i.test(line))
...
function validateMissionQueueStore(store) {
  ...
  for (const job of store.jobs) {
    ...
    for (const attempt of job.attempts) {
      ...
      if (attempt.confirmationMarker != null && attempt.confirmationMarker !== `[Host Control Dispatch ${attempt.id}]`) {
        throw new Error('mission_queue_job_attempt_invalid');
      }
...
function missionNotificationItem(event, job, disposition = {}) {
  ...
  const details = {
    ...
    verification_ready: 'Review the reported result and evidence. Host Control will never mark it Done automatically.'
  };
  return {
    id,
    dedupeKey: event.id,
...
async function createMission(body, req) {
  ...
  return enqueueMissionOperation(async () => {
    ...
    if (!missionDispatchPrompt(job, '[Host Control Dispatch attempt-0000000000-00000000]')) {
...
function reviewPrompt(meta) {
  return [
    'You are the dedicated Host Control review agent.',
    '',
  ...
  const security = {
    ...
    warnings: CONTROL_PLANE_MODE === 'tmux-legacy'
      ? [{
          ...
          title: 'Host Control still shares the workload tmux server',
...
function missionDispatchPrompt(job, confirmationMarker = '') {
  ...
  const prompt = [
    `[Host Control Mission ${job.id}]`,
    `Outcome: ${oneLine(job.title)}`,
    `Workspace: ${oneLine(job.workspace)}`,
    `Goal: ${oneLine(job.goal)}`,
    `Verification required before Host Control can mark this Done: ${oneLine(job.verificationCriteria)}`,
    ...
    'When you stop, report a concise STATUS, RESULT, EVIDENCE, and NEXT ACTION in this terminal. Host Control will keep completion behind a separate verification gate.',
    confirmationMarker
  ].filter(Boolean).join(' | ');
...
async function dispatchMission(id, body, req) {
  return enqueueMissionOperation(async () => {
    ...
    const attemptId = `attempt-${Date.now().toString(36)}-${randomBytes(4).toString('hex')}`;
    const confirmationMarker = `[Host Control Dispatch ${attemptId}]`;
    ...
    if (delivery.ok) {
      ...
      job.blocker = delivery.stage === 'literal_confirmation'
        ? 'Mission text may be present, but Host Control did not send Enter because the full input was not confirmed. Inspect the terminal before retrying.'
  ...
  if (prompt.trim()) {
    ...
    if (!pane) {
      ...
      if (!ready.ok) {
        ...
        const startMarker = `[Host Control Initial Prompt ${markerToken} Start]`;
        const marker = `[Host Control Initial Prompt ${markerToken} End]`;
...
server.listen(PORT, HOST, () => {
  console.log(`Host Control listening on http://${HOST}:${PORT}`);
});
