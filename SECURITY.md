...
- Any non-loopback listener requires HTTP Basic authentication before serving the application or issuing a control cookie. The username is always `host-control`; the password is an operator token of at least 24 characters.
...
If `ORCHESTRATOR_ACCESS_TOKEN` is not supplied for a non-loopback listener, Host Control creates a random owner-only token at `data/access-token`. Reveal it locally with `bash scripts/show-access-token.sh`. Never paste the token into chat, logs, issues, or shell history.
