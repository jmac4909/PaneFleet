...
function validIdeaItem(overrides = {}) {
  const timestamp = '2026-07-16T00:00:00.000Z';
  return {
    id: 'idea-integrity-12345678',
    revision: 1,
    status: 'proposed',
    title: 'Synthetic idea title',
    details: 'synthetic-private-idea-details',
    source: 'operator',
    sourceSession: '',
    sourcePromptId: null,
...
async function rejectedPersistedState(files) {
  ...
  try {
    const [code, signal] = await waitForChildExit(child, {
      timeoutMs: 5000,
      label: 'invalid persisted state startup'
    });
    return {
      code,
      signal,
  ...
  const cases = [
    ...
    },
    {
      ...
      files: {
        'mission-queue.json': JSON.stringify(emptyMissionQueue()),
        'prompt-queue.json': JSON.stringify(emptyPromptQueue()),
        'notification-state.json': JSON.stringify({ version: 1, revision: 0, items: {} }),
        'network-monitor.json': malformed
      },
      error: 'network_monitor_state_json_invalid'
    },
    {
      name: 'Codex usage history',
      files: {
        'mission-queue.json': JSON.stringify(emptyMissionQueue()),
        'prompt-queue.json': JSON.stringify(emptyPromptQueue()),
        'notification-state.json': JSON.stringify({ version: 1, revision: 0, items: {} }),
        'codex-usage-history.json': malformed
      },
      error: 'codex_usage_state_json_invalid'
    }
  ];
...
test('durable mission stores reject corrupt shape and job records before any host inspection', async (t) => {
  ...
  const finishedAt = '2026-07-16T00:05:00.000Z';
  const completionEvidence = 'Focused isolated validation passed.';
  const job = (root, overrides = {}) => validMissionJob(root, { goal: privateMarker, ...overrides });
  const terminalJob = (root, status, overrides = {}) => job(root, {
    status,
    finishedAt,
    resultSummary: status === 'canceled' ? '' : completionEvidence,
    verification: status === 'done'
      ? { status: 'passed', note: completionEvidence, at: finishedAt }
      : { status: 'pending', note: '', at: null },
    outcomes: [{ status, note: completionEvidence, at: finishedAt }],
    ...overrides
  });
  ...
  const cases = [
    ...
    { name: 'done mission requires durable completion proof', build: (root) => store(root, [job(root, { status: 'done' })]), error: /mission_queue_job_lifecycle_invalid/ },
    { name: 'done mission verification time matches finish time', build: (root) => store(root, [terminalJob(root, 'done', { verification: { status: 'passed', note: completionEvidence, at: '2026-07-16T00:06:00.000Z' } })]), error: /mission_queue_job_lifecycle_invalid/ },
    { name: 'done mission proof fields agree', build: (root) => store(root, [terminalJob(root, 'done', { resultSummary: 'Different claimed result.' })]), error: /mission_queue_job_lifecycle_invalid/ },
    { name: 'failed mission requires matching terminal outcome', build: (root) => store(root, [terminalJob(root, 'failed', { outcomes: [] })]), error: /mission_queue_job_lifecycle_invalid/ },
    { name: 'canceled mission requires matching terminal outcome', build: (root) => store(root, [terminalJob(root, 'canceled', { outcomes: [] })]), error: /mission_queue_job_lifecycle_invalid/ },
    { name: 'nonterminal mission cannot retain finish time', build: (root) => store(root, [job(root, { finishedAt })]), error: /mission_queue_job_lifecycle_invalid/ },
    { name: 'nonterminal mission cannot claim passed verification', build: (root) => store(root, [job(root, { verification: { status: 'passed', note: completionEvidence, at: finishedAt } })]), error: /mission_queue_job_lifecycle_invalid/ },
...
test('durable prompt stores reject corrupt shape and item fields before any host inspection', async (t) => {
  ...
  const cases = [
    ...
    { name: 'oversized prompt', store: validStore([validPromptItem({ text: 'x'.repeat(30001) })]), error: /prompt_queue_item_text_invalid/ },
    ...
    { name: 'oversized completion snapshot', store: validStore([validPromptItem({ completionSnapshot: 'x'.repeat((32 * 1024) + 1), text: privateMarker })]), error: /prompt_queue_item_completion_invalid/ },
    { name: 'invalid idea proposal count', store: validStore([validPromptItem({ ideaProposalCount: 13, text: privateMarker })]), error: /prompt_queue_item_completion_invalid/ },
    ...
    { name: 'invalid acknowledged superseding interaction timestamp', store: validStore([validPromptItem({ supersedingInteractionAcknowledgedAt: 'not-a-time', text: privateMarker })]), error: /prompt_queue_item_timestamp_invalid/ },
...
test('durable idea queue rejects malformed proposals and impossible approval lifecycles', async (t) => {
  const privateMarker = 'synthetic-private-idea-details';
  const validStore = (ideas, overrides = {}) => ({
    version: 1,
    revision: 1,
    items: [],
    schedules: [],
  ...
  const idea = (overrides = {}) => validIdeaItem({ details: privateMarker, ...overrides });
  const cases = [
    { name: 'idea collection is an array', store: validStore([], { ideas: {} }), error: /idea_queue_shape_invalid/ },
    { name: 'idea collection is bounded', store: validStore(Array.from({ length: 201 }, (_, index) => ({ id: index }))), error: /idea_queue_shape_invalid/ },
    { name: 'idea record is an object', store: validStore([null]), error: /idea_queue_item_invalid/ },
    { name: 'idea id is validated', store: validStore([idea({ id: 'idea-short' })]), error: /idea_queue_item_invalid/ },
    { name: 'idea status is validated', store: validStore([idea({ status: 'running' })]), error: /idea_queue_item_invalid/ },
    { name: 'idea title is required', store: validStore([idea({ title: '  ' })]), error: /idea_queue_title_invalid/ },
    { name: 'idea details are bounded', store: validStore([idea({ details: 'x'.repeat(3001) })]), error: /idea_queue_details_invalid/ },
    { name: 'agent source requires a session', store: validStore([idea({ source: 'agent' })]), error: /idea_queue_source_invalid/ },
    { name: 'agent source requires a prompt ticket', store: validStore([idea({ source: 'agent', sourceSession: 'codex-worker' })]), error: /idea_queue_source_invalid/ },
    { name: 'operator source cannot claim a session', store: validStore([idea({ sourceSession: 'codex-worker' })]), error: /idea_queue_source_invalid/ },
    { name: 'operator source cannot claim a prompt ticket', store: validStore([idea({ sourcePromptId: 'prompt-source-12345678' })]), error: /idea_queue_source_invalid/ },
    { name: 'agent source ticket has trustworthy completion proof', store: validStore([idea({ source: 'agent', sourceSession: 'codex-worker', sourcePromptId: 'prompt-source-12345678' })], { items: [validPromptItem({ id: 'prompt-source-12345678' })] }), error: /idea_queue_source_invalid/ },
    { name: 'agent source ticket belongs to its attributed session', store: validStore([idea({ source: 'agent', sourceSession: 'codex-other', sourcePromptId: 'prompt-source-12345678' })], { items: [validSentPromptItem({ id: 'prompt-source-12345678', summaryState: 'captured', completionSummary: 'Synthetic captured response.', completionSnapshot: 'Synthetic captured response.', completedAt: '2026-07-16T00:00:00.000Z' })] }), error: /idea_queue_source_invalid/ },
    { name: 'idea prompt links are validated', store: validStore([idea({ refinementPromptId: 'prompt-short' })]), error: /idea_queue_link_invalid/ },
    { name: 'refining idea has a refinement ticket', store: validStore([idea({ status: 'refining' })]), error: /idea_queue_lifecycle_invalid/ },
    { name: 'refining idea has a linked open ticket', store: validStore([idea({ status: 'refining', refinementPromptId: 'prompt-refinement-12345678' })]), error: /idea_queue_lifecycle_invalid/ },
    { name: 'open refinement ticket is the current parent link', store: validStore([idea()], { items: [validPromptItem({ ideaId: 'idea-integrity-12345678', ideaPurpose: 'refinement' })] }), error: /idea_queue_lifecycle_invalid/ },
    { name: 'open approved ticket has its parent idea', store: validStore([], { items: [validPromptItem({ ideaId: 'idea-missing-12345678', ideaPurpose: 'approved' })] }), error: /idea_queue_lifecycle_invalid/ },
    { name: 'approved idea has an implementation ticket', store: validStore([idea({ status: 'approved', resolvedAt: '2026-07-16T00:00:00.000Z' })]), error: /idea_queue_lifecycle_invalid/ },
    { name: 'rejected idea cannot retain an approval ticket', store: validStore([idea({ status: 'rejected', approvedPromptId: 'prompt-approved-12345678', resolvedAt: '2026-07-16T00:00:00.000Z' })]), error: /idea_queue_lifecycle_invalid/ },
    { name: 'resolved idea requires resolution time', store: validStore([idea({ status: 'rejected' })]), error: /idea_queue_lifecycle_invalid/ },
    { name: 'idea timestamps are validated', store: validStore([idea({ updatedAt: 'not-a-time' })]), error: /idea_queue_timestamp_invalid/ },
    { name: 'idea chronology is monotonic', store: validStore([idea({ createdAt: '2026-07-16T00:01:00.000Z' })]), error: /idea_queue_chronology_invalid/ }
  ];
...
test('durable recurring schedules reject corrupt definitions before any host inspection', async (t) => {
  ...
  const cases = [
    ...
    {
      name: 'duplicate schedule definition',
      store: validStore([schedule(), schedule({ id: 'schedule-equivalent-87654321' })]),
      error: /prompt_schedule_duplicate_definition/
    },
    ...
    { name: 'oversized recurring prompt', store: validStore([schedule({ text: 'x'.repeat(30001) })]), error: /prompt_schedule_text_invalid/ },
    ...
    { name: 'impossible calendar schedule', store: validStore([schedule({ cron: '0 0 30 2 *' })]), error: /prompt_schedule_has_no_run/ },
    ...
    { name: 'next run must match cron', store: validStore([schedule({ nextRunAt: '2026-07-16T00:06:00.000Z' })]), error: /prompt_schedule_next_run_invalid/ },
