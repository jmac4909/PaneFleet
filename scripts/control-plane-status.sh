...
printf 'isolation=%s unit=%s enabled=%s active=%s pid=%s listener_pid=%s health=%s linger=%s legacy_tmux=%s workload_tmux=%s workloads=%s\n' \
  "$isolation" "$UNIT" "${enabled:-unknown}" "${active:-unknown}" "${main_pid:-0}" \
  "${listener_pid:-0}" "$health" "${linger:-unknown}" "$legacy" "$workload_tmux" "$workloads"
