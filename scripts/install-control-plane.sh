...
[[ "$CURRENT_USER" =~ ^[A-Za-z_][A-Za-z0-9_.-]{0,127}$ ]] || { printf 'could not determine current user\n' >&2; exit 2; }
...
sudo -n loginctl enable-linger "$CURRENT_USER"
linger="$(loginctl show-user "$CURRENT_USER" -p Linger --value)"
